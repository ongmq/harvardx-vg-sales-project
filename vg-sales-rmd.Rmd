---
title: "Kaggle Video Games Sales Project"
author: "ongmq"
date: "2022-09-06"
output: pdf_document
---
...
```{r installing R packages, message=FALSE}
...
```{r inspect rating variable, echo=FALSE}
vg_sales %>% count(Rating)
```

```{r merge rating categories}
...
```{r unique rateing categories, echo=FALSE}
...
```{r video games by platform, echo=FALSE}
vg_sales %>% 
  group_by(Platform) %>%
  count() %>%
...
```{r video games by genre, echo=FALSE}
vg_sales %>% 
  group_by(Genre) %>%
  count() %>%
  ggplot(aes(Genre, n)) +
...
```{r video games by rating, echo=FALSE}
vg_sales %>% 
  group_by(Rating) %>%
  count() %>%
  ggplot(aes(Rating, n)) +
...
```{r video games by year of release, echo=FALSE}
vg_sales %>% 
...
```{r video games by critic score, echo=FALSE}
vg_sales %>% 
...
```{r video games by user score, echo=FALSE}
vg_sales %>% 
...
```{r video games by publisher, echo=FALSE}
vg_sales %>% 
  group_by(Publisher) %>%
  count() %>%
  arrange(desc(n)) %>%
...
```{r video games by developer, echo=FALSE}
vg_sales %>% 
  group_by(Developer) %>%
  count() %>%
  arrange(desc(n)) %>%
...
```{r video games by global sales, echo=FALSE}
vg_sales %>% 
...
```{r global sales by platform, echo=FALSE}
vg_sales %>%
...
```{r global sales by genre, echo=FALSE}
vg_sales %>%
...
```{r global sales by rating, echo=FALSE}
vg_sales %>%
...
```{r global sales by year of release, echo=FALSE}
vg_sales %>%
...
```{r global sales by critic score, echo=FALSE}
vg_sales %>%
...
```{r global sales by user score, echo=FALSE}
vg_sales %>%
...
```{r global sales by publisher, echo=FALSE}
vg_sales %>%
  group_by(Publisher) %>%
...
```{r global sales by developer, echo=FALSE}
vg_sales %>%
  group_by(Developer) %>%
  summarise(sales = sum(Global_Sales)) %>%
  arrange(desc(sales)) %>%
...
```{r create validation set, warning=FALSE}
...
```{r join validation set, message=FALSE}
validation_set <- temp %>% 
...
```{r add remainder set, message=FALSE}
...
```{r create train and test sets, warning=FALSE}
set.seed(1, sample.kind = "Rounding")
...
```{r remove datasets, include=FALSE}
...
The performance of the algorithm will be determined by its root mean square error (RMSE), with smaller
RMSE indicating better performance.

```{r lm model, warning=FALSE}
